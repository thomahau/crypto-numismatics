...
App.UI = {
  ...
  renderStartPage: function() {
    $('.portfolio-container, .js-username, .js-logout, .js-register-success').attr('hidden', true);
    $(
  ...
  renderPortfolio: function() {
    ...
    $('.welcome-container, .search-help, .stats-wrapper, .particles-js-canvas-el').attr(
      'hidden',
      true
    );
  ...
  getPortfolioHeader: function(populatedHoldings) {
    const symbol = App.Lib.getCurrencySymbol(localStorage.getItem('currency'));
      ...
      populatedHoldings.forEach(holding => {
        ...
        tableRowsHtmlString += `
          ...
          holding.percent_change_7d
              ...
              holding.name
            }" data-coin="${holding.id}"><i class="fas fa-trash-alt"></i>
						</a>
					</td>
				</tr>`;
      });
  ...
  handleCancelAdditionBtn: function(populatedHoldings) {
    $('main').on('click', '.cancel-addition-btn', function() {
      const portfolioFooter = App.UI.getPortfolioFooter(populatedHoldings);
  ...
  validateInput: function(input) {
    return new Promise((resolve, reject) => {
      availableCoins.includes(input) ? resolve() : reject('Invalid coin name');
    });
  },
      ...
      holdingsHtmlString += `
			<div class="row">
				...
				<div class="nine columns">
					<input type="number" name="${item.id}" id="${item.name}" value=${item.amount} min="0" step="any" />
					<a class="button delete-holding" role="button" tabindex="0" data-coin="${item.id}">Delete</a>
				</div>
			</div>`;
    });
    ...
    while (switching) {
      ...
      for (i = 1; i < rows.length - 1; i++) {
        ...
        let xValue = App.Lib.formatForSort(
          x.firstElementChild ? x.firstElementChild.textContent : x.textContent
        );
        let yValue = App.Lib.formatForSort(
          y.firstElementChild ? y.firstElementChild.textContent : y.textContent
        );
